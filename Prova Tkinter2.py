...
class Game(tk.Frame):
    def __init__(self, master, size=(610, 400)):
...
def top_right_corner(size, window):
    w = size[0]
    h = size[1]
    x = (ws - w - 8)
    y = 0
...
class Brick(GameObject):
    COLORS = {1: '#710000', 2: '#Da2e2e', 3: '#D2d2d2'}
    ...
    def __init__(self, canvas, x, y, hits):
        ...
        item = canvas.create_rectangle(x - width / 2,
                                       y - height / 2,
                                       x + width / 2,
                                       y + height / 2,
                                       fill=color,
...
if __name__ == '__main__':
    ...
    if ne:
        scale = 4/11
        window_size = (int(scale*ws), int(scale*hs))
        top_right_corner(window_size, root)
